...
function DailyMedications() {
  ...
  const addTask = () => {
    if (newTask.trim() && newTime) {
      fetch('http://localhost/my-medi-app/src/php/pill.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newTask, time: newTime, note: newNote })
      })
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h1 className="text-2xl font-bold mb-4">Pill Reminders</h1>
      <ul>
        {tasks.map((task) => (
